...
if __name__ == '__main__':
  ...
  flags.DEFINE_integer("num_readers",100,
...
def inference(reader, train_dir, data_pattern, out_file_location, batch_size, top_k):
  with tf.Session(config=config) as sess, gfile.Open(out_file_location, "w+") as out_file:
    video_id_batch, video_batch, num_frames_batch = get_input_data_tensors(reader, data_pattern, batch_size,num_readers=FLAGS.num_readers)
...
def main(unused_argv):
  ...
  inference(reader, FLAGS.train_dir, '../input/frames/test*',
